...
import { DEFAULT_ANIME_LIST, GENRE_MAP } from "@/public/data/meta";
import axios from "axios";
import { BadgeCheckIcon, Loader, Search } from "lucide-react";
import { toast } from "sonner";
import { fetchGenres } from "@/lib/fetchGenres";
import Image from "next/image";
import { getRandomSuggestions } from "@/public/data/suggestions";
import GenreSection from "@/components/custom/genreSection";
...
const HomeComponent = () => {
  ...
  const { current: suggestions } = useRef(getRandomSuggestions());
  ...
  useEffect(() => {
    events.on("loadAnime", getAnime);

    return () => {
      events.off("loadAnime");
    };
  }, []);
      ...
      <div className="relative z-10">
        <main className="flex flex-col items-center justify-items-center min-h-screen gap-16 p-10 md:p-40 xl:p-64">
          ...
          <div className="flex flex-col items-center justify-items-cente gap-8 w-[60%]">
            ...
            {/* Prompt Suggestions
             * Shown only if it's a new search tab
             */}
            <div className="flex flex-wrap justify-center gap-2">
              {detectedGenres.length === 0 &&
                animeResults.results.length === 0 &&
            ...
            {/* Show Detected Genres from prompt */}
            {detectedGenres.length > 0 && (
              <GenreSection initialGenres={detectedGenres} />
            )}

            {/* Show Anime Recommendations from prompt */}
